...
def test_unified():
    ...
    print(f"{'REQ_ID':<12} | {'EXPECTED METHOD':<18} | {'CALCULATED METHOD':<18} | {'EXPECTED STATUS':<18} | {'CALCULATED STATUS':<18} | {'MATCH?'}")
    print("-" * 115)
    ...
    for req in sample_requests:
        ...
        print(f"{rid:<12} | {str(exp_method):<18} | {str(calc_method):<18} | {str(exp_status):<18} | {str(calc_status):<18} | {is_match}")

    print(f"\nUnified 25-Sample Method + Status Matches: {matches} / {total}")
