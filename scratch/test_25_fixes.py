...
# Refine find_next_salary_date: return None if no salary events exist
def find_next_salary_date_fixed(user_events, request_date_str, profile=None, user_messages=None):
    salary_events = [
    ...
    past_salaries = [
        ...
        if (e.settlement_date and e.settlement_date <= request_date_str) or e.event_date <= request_date_str
    ]
    if past_salaries:
...
se.find_next_salary_date = find_next_salary_date_fixed
opt.safe_engine.find_next_salary_date = find_next_salary_date_fixed

# Refine find_spending_changes: allow future stoppable/reducible events
...
print("Testing 25 sample evaluation with fixes...")
ms_matches = 0
